...
public static void main(String[] args) {
	...
	System.out.println(stk.nextToken());
	System.out.println(stk.nextToken());
	System.out.println("hasMoreToken()?"+stk.hasMoreTokens());
	...
	while(stk.hasMoreTokens()) {
		...
		System.out.println("생성된 Score의 객체:"+score);
		...
		for (int i = 0; i < result.length; i+=4) {
			Score s = new Score(result[i+0], Integer.parseInt(result[i+1]),Integer.parseInt(result[i+2]),Integer.parseInt(result[i+3]));
			scores.add(s);
		}
